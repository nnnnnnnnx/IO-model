...
class Percentile{
    ...
    private:
        size_t max_stat_data_len_{1024};//原始统计数据更大长度
        std::map<std::string, std::vector<int64_t> > origin_stat_data_; //原始统计长度
        std::map<std::string, int32_t> origin_stat_data_index_; //原始统计数据的索引
};