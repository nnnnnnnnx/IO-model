    ...
    class Codec{
        public:
            ...
            void EnCode(const std::string &msg, Packet &pkt){
                ...
                *(uint32_t *)pkt.Data() = htonl(msg.length()); //将协议体的长度转换为网络字节序
                memcpy(pkt.Data() + 4, msg.data(), msg.length());
            }
            void Decode(uint8_t *data, size_t len){
                uint32_t decodeLen = 0; //本次解析的长度
        ...
        private:
            ...
            std::string reserved_; //为解析的网络字节流
